...
Set Decay Window to 1 hour (minimum allowed).
Then set DECAY_CHECK_INTERVAL_MS=10000 to see 
it decay within seconds of the hour passing.
...
1. Create a job with capacity 1 and decay window 1 hour
2. Apply with Applicant A → becomes active
3. Apply with Applicant B → becomes waitlisted
4. Reject Applicant A → Applicant B auto-promotes (promoted_at is set)
5. Login as Applicant B → see live countdown timer ticking down
6. Wait without acknowledging → scheduler fires → Applicant B decays back to waitlisted
7. Check Applicant B status → waitlisted with decay_count=1, decay_level=1
8. View Audit Trail → see full history of every transition
