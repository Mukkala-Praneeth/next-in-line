...
When creating a job on the Company Dashboard, set Decay Window to 1 (minimum: 1 hour).
...
DECAY_CHECK_INTERVAL_MS=10000
This makes the scheduler check every 10 seconds instead of 60.

Test flow:
Create a job with capacity 1 and decay window 1 hour
Apply with Applicant A — becomes active
Apply with Applicant B — becomes waitlisted
Reject Applicant A — Applicant B auto-promotes (promoted_at is set)
Login as Applicant B — see live countdown timer ticking down
Wait without acknowledging — scheduler fires — Applicant B decays back to waitlisted
Check Applicant B status — waitlisted with decay_count=1, decay_level=1
View Audit Trail — see full history of every transition
Architecture
Database Schema
text
...
For each: decays them, promotes the next person, logs everything
If the newly promoted person also does not respond, they get caught in the next cycle
Cascade continues automatically with zero manual intervention
...
Interviewer Assignment — Distribute active applicants evenly across team members to balance review workload
Email Notifications — Alert applicants on promotion, approaching decay window, and status changes
WebSocket Integration — Real-time pipeline updates on the company dashboard
Bulk Actions — Reject or advance multiple applicants at once
...
Testing — Unit tests for pipelineService, integration tests for concurrent submissions
Docker Compose — One-command local setup with PostgreSQL, backend, and frontend
Graceful Shutdown — Stop the decay scheduler and drain DB connections on SIGTERM
API Documentation
See APIDOCS.md for complete endpoint documentation with inputs, outputs, and error codes.
